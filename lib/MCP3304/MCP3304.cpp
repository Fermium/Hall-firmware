...
int MCP3304::readAdc(int ch, boolean sgl) {
								...
								SPI.transfer(configBits | (ch >> 1)); //4bits 0, startbit, SGL/DIFF, 2 Channelbits (D2, D1)
								byte hi = SPI.transfer(ch << 7); //lowest Channelbit (D0), rest dont care;return= first 2 returnbits undef, nullbit, Signbit, 4 highest databits
...
int MCP3304::readSgl(int ch) {
								...
								SPI.transfer(0x0C | (ch >> 1)); //4bits 0, startbit, SGL = 1, 2 Channelbits (D2, D1)
								byte hi = SPI.transfer(ch << 7); //lowest Channelbit (D0), rest dont care; return= first 2 returnbits undef, nullbit, Signbit, 4 highest databits
								byte lo = SPI.transfer(0x00); //send dont care; return= 8 lowest databits
...
int MCP3304::readDiff(int ch) {
								...
								SPI.transfer(0x08 | (ch >> 1)); //4bits 0, startbit, DIFF = 0, 2 Channelbits (D2, D1)
								byte hi = SPI.transfer(ch << 7); //lowest Channelbit (D0), rest dont care; return= first 2 returnbits undef, nullbit, Signbit, 4 highest databits
								byte lo = SPI.transfer(0x00); //send dont care; return= 8 lowest databits
